...
class Entity1(Entity):
    ...
    def __init__(self, sim, startvector):
        ...
        # Diagonal work done here
        self.distance_table[0][0] = self.sim.cost[0][1]
        self.distance_table[1][1] = self.sim.cost[1][1]
        self.distance_table[2][2] = self.sim.cost[2][1]
        self.distance_table[3][3] = self.sim.cost[3][1]

        # Similar reasoning with Entity0 commenting
    ...
    def start(self):
        ...
        # Packets being sent
        packet1 = Packet(self.sim, 1, 0, self.distance_table)
        self.sim.to_layer2(1, packet1)
        packet2 = Packet(self.sim, 1, 2, self.distance_table)
        self.sim.to_layer2(1, packet2)
    ...
    def link_cost_change_handler(self, which_link, new_cost):
        ...
        of that path"""
        # Attempted change handler
        self.distance_table[which_link][which_link] = new_cost
        self.pkt_minCost[which_link] = self.distance_table[which_link][which_link]

        packet1 = Packet(self.sim, 1, 0, self.pkt_minCost[which_link])
        self.sim.to_layer2(1, packet1)
        packet2 = Packet(self.sim, 1, 2, self.pkt_minCost[which_link])
        self.sim.to_layer2(1, packet2)
